...
mod tests {
    ...
    #[test]
    fn test_parse_tauri_format() {
        let hotkey = Hotkey::parse("Ctrl+Alt+KeyW").unwrap();
        assert!(hotkey.ctrl);
        assert!(hotkey.alt);
        assert_eq!(hotkey.key, Key::KeyW);
    }
    ...
    fn test_parse_macos_format() {
        let hotkey = Hotkey::parse("cmd+option+w").unwrap();
        assert!(hotkey.super_key);
        assert!(hotkey.alt);
        assert_eq!(hotkey.key, Key::KeyW);
    }
    ...
    #[test]
    fn test_is_pressed_with_left_modifiers() {
        let hotkey = Hotkey::parse("ctrl+alt+w").unwrap();
        let mut pressed = HashSet::new();
        pressed.insert(Key::ControlLeft);
        pressed.insert(Key::Alt);
        pressed.insert(Key::KeyW);
        assert!(hotkey.is_pressed(&pressed));
    }

    #[test]
    fn test_is_pressed_with_right_modifiers() {
        let hotkey = Hotkey::parse("ctrl+shift+w").unwrap();
        let mut pressed = HashSet::new();
        pressed.insert(Key::ControlRight);
        pressed.insert(Key::ShiftRight);
        pressed.insert(Key::KeyW);
        assert!(hotkey.is_pressed(&pressed));
    }

    #[test]
    fn test_is_pressed_missing_modifier() {
        let hotkey = Hotkey::parse("ctrl+alt+w").unwrap();
        let mut pressed = HashSet::new();
        pressed.insert(Key::ControlLeft);
        // Missing Alt
