...
mod tests {
    ...
    fn test_round_trip() {
        let mut settings = Settings::default();
        settings.transcription.provider = TranscriptionProvider::OpenAI;
        ...
        settings.ui.active_preset = Some("formal".to_string());
        settings.ui.vad.enabled = true;
        settings.ui.vad.threshold = 0.7;
        settings.services.ollama.url = Some("http://localhost:11434".to_string());

        // Convert to map and back
        let map = settings.to_store_map();
        let restored = Settings::from_store_map(&map);

        assert_eq!(
            restored.transcription.provider,
            TranscriptionProvider::OpenAI
        );
        assert_eq!(restored.transcription.language, Some("en".to_string()));
        assert_eq!(
            restored.transcription.api_keys.get("openai"),
            Some(&"sk-test".to_string())
        );
        assert_eq!(restored.post_processing.processor, PostProcessor::OpenAI);
        assert_eq!(restored.ui.active_preset, Some("formal".to_string()));
        assert!(restored.ui.vad.enabled);
        assert_eq!(restored.ui.vad.threshold, 0.7);
        assert_eq!(
            restored.services.ollama.url,
    ...
    fn test_api_key_store_key() {
        assert_eq!(
            Settings::api_key_store_key("openai"),
            Some("openai_api_key")
        );
        assert_eq!(
            Settings::api_key_store_key("openai-realtime"),
            Some("openai_api_key")
        );
        assert_eq!(
            Settings::api_key_store_key("deepgram"),
            Some("deepgram_api_key")
        );
        assert_eq!(
            Settings::api_key_store_key("deepgram-realtime"),
            Some("deepgram_api_key")
        );
        assert_eq!(Settings::api_key_store_key("unknown"), None);
    }

    #[test]
    fn test_empty_map_uses_defaults() {
        let map = HashMap::new();
        let settings = Settings::from_store_map(&map);

        // Should use defaults
        assert_eq!(
            settings.transcription.provider,
            crate::configuration::DEFAULT_PROVIDER
        );
        assert_eq!(settings.post_processing.processor, PostProcessor::None);
    }
}
